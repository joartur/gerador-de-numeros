// src/app/components/BingoGame.tsx - Atualizado
...
export default function BingoGame({ onBack }: BingoGameProps) {
  ...
  // Gera cartelas aleatórias seguindo padrão BINGO
  const generateCards = () => {
    setError("");
    setGameWinners([]);
    ...
    for (let i = 0; i < numberOfCards; i++) {
      ...
      // Distribui os números proporcionalmente entre as colunas B, I, N, G, O
  ...
  const handleWinnerFound = (winners: number[]) => {
            ...
            {gameWinners.length > 0 && (
              ...
              >
                ...
                <strong style={{ color: "#f59e0b" }}>
                  Último sorteio - Vencedor(es): Cartela(s) {gameWinners.map(w => `#${w + 1}`).join(", ")}
                </strong>
              </div>
            )}
          </div>
